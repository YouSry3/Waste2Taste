...
public class ProductService : IProductService
{
    ...
    // ═══════════════════════════════════════════════════════════════════
    // FIX: Use UtcNow consistently and auto-check expiry
    // ═══════════════════════════════════════════════════════════════════
    private static string CalculateExpiresIn(DateTime expiryDate)
    {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // NEW: Auto-check and update expired status based on date
    // ═══════════════════════════════════════════════════════════════════
    ...
    public async Task<Result<IEnumerable<ProductListResponse>>> GetAllAsync(string? name)
    {
        ...
        foreach (var product in products)
        {
            // Auto-update expired flag if date has passed
            if (!product.Expired && IsExpired(product.ExpiryDate))
            {
                product.Expired = true;
                await _productRepository.UpdateAsync(product);
            }
    ...
    public async Task<Result> UpdateAsync(Guid id, UpdateProductRequest request)
    {
        ...
        {
            product.ExpiryDate = request.ExpiryDate.Value;
            // Auto-update expired flag when expiry date changes
            product.Expired = IsExpired(product.ExpiryDate);
        }

        // ═══════════════════════════════════════════════════════════════
        // FIX: Vendors can no longer manually set Expired via Update
        // Expiry is determined by date only
        // ═══════════════════════════════════════════════════════════════
        // REMOVED: if (request.Expired.HasValue) product.Expired = request.Expired.Value;
    ...
    // ═══════════════════════════════════════════════════════════════════
    // FIX: MarkAsExpired now checks if date actually passed
    // ═══════════════════════════════════════════════════════════════════
    public async Task<Result> MarkAsExpiredAsync(Guid id)
    {
        var product = await _productRepository.GetByIdAsync(id);
        ...
        if (!IsExpired(product.ExpiryDate))
            return Result.Failure(ProductErrors.NotYetExpired);
