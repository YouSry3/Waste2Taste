...
export const ordersApi = {
  getOrders: async (filters: any = {}, pagination: any = { page: 1, limit: 10 }) => {
  ...
  getOrderById: async (orderId: string) => {
    const res = await apiClient.get(`/api/dashboard/orders/${orderId}`);
    return res.data;
  },

   updateOrderStatus: async (orderId: string, status: string) => {
    //  orderId is now the raw GUID
    const res = await apiClient.patch(`/api/dashboard/orders/${orderId}/status`, { status });
    return res.data;
  },

  exportOrders: async (format: string, filters?: any) => {
