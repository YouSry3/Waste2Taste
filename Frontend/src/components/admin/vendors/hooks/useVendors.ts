...
//     openEditDialog,
//     openDeleteDialog,
//     handleToggleBlock: useCallback((vendor: Vendor) => {
//   setHasLocalChanges(true);
//   actions.toggleBlock(vendor);
// }, [actions]),
...
export const useVendors = () => {
  ...
  // ── Fix: set hasLocalChanges so the useEffect doesn't overwrite
  //         the optimistic isBlocked update after query invalidation
  ...
  return {
    ...
    openEditDialog,
    openDeleteDialog,
    handleToggleBlock,  // ← now correctly sets hasLocalChanges
    isBlocking: actions.isBlocking,
