...
import toast from "react-hot-toast";
import { useModerationData } from "./useModerationData";
import { ModerationAction, ModerationActionType, ModerationId } from "../types";
...
export function useModerationActions() {
  const {
    ...
    setCustomerReports,
    addToActivityLog,
    handleApproveVendor: dataApproveVendor,
    handleRejectVendor: dataRejectVendor,
  } = useModerationData();
  ...
  // ─── Vendor Actions ────────────────────────────────────────────────────────
  // These delegate to useModerationData which calls the correct backend endpoint:
  ...
  const handleRejectVendor = async (
    ...
    try {
      await dataRejectVendor(id, { reason, notes });
  ...
  // Bulk vendor actions
  const handleBulkApproveVendors = async (selectedVendors: ModerationId[]) => {
    setLoading("bulk-approve-vendors", true);
    ...
    try {
      ...
      await Promise.all(selectedVendors.map((id) => dataApproveVendor(id)));
  ...
  const handleBulkRejectVendors = async (selectedVendors: ModerationId[]) => {
    setLoading("bulk-reject-vendors", true);
    ...
    try {
      ...
      await Promise.all(
        selectedVendors.map((id) =>
          dataRejectVendor(id, { reason: "Bulk rejection", notes: "" }),
        ),
      );
