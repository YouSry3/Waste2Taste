...
import {useAllReports} from "../services/adminReportsService";
import {
  Listing,
  ModerationId,
  VendorRequest,
...
const getRequestFallbackId = (request: any, index: number): ModerationId => {
  const idValue =
    request?.id ??
    request?.Id ??
    request?.requestId ??
...
const dedupeVendorRequestsById = (requests: VendorRequest[]) => {
  const seen = new Set<ModerationId>();
...
const { data: apiReports = [] } = useAllReports();
  ...
  // Vendor actions — call the real backend endpoint via vendorRequestService
  ...
  const handleRejectVendor = async (id: ModerationId, data: RejectRequest) => {
    const vendor = vendorRequests.find((item) => item.id === id);
  ...
  const handleBulkApproveVendors = async (ids: ModerationId[]) => {
    const vendorsById = new Map(
      vendorRequests.map((vendor) => [vendor.id, vendor] as const),
    );

    try {
      await Promise.all(ids.map((id) => approveVendorRequest(id)));
  ...
  const handleBulkRejectVendors = async (
    ...
    try {
      await Promise.all(
        ids.map((id) =>
          rejectVendorRequest(id, { reason: data.reason, notes: data.notes }),
        ),
      );
...
useEffect(() => {
  if (isDemoMode || !apiReports.length) return;

  const mapped: CustomerReport[] = apiReports.map((r) => ({
    id: r.rawId as any,
    reporter: r.customer,
    orderId: r.orderId,
    vendor: r.vendor,
  ...
  setCustomerReports(mapped);
}, [isDemoMode, apiReports]);
  return {
